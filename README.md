...
React and Node.js dashboards for evidence-led YouTube video and channel analysis. The web app uses official YouTube APIs; `AgentTests/` is the older Python/`yt-dlp` prototype. For full architecture, OAuth publishing, and deployment guidance, see [WEBAPP\_HANDOVER.md](docs/WEBAPP_HANDOVER.md).
...
* **Channel dashboard:** whole-catalogue public metrics, upload-age and duration cohorts, reach/engagement outliers, sortable upload data, and one evidence-linked bounded GPT interpretation.
...
YOUTUBE\_API\_KEY=...
OPENAI\_API\_KEY=...
OPENAI\_VIDEO\_MODEL=gpt-5.4
OPENAI\_CHANNEL\_MODEL=gpt-5.4
```
...
GOOGLE\_OAUTH\_REDIRECT\_URI=http://localhost:3000/auth/google/callback
SESSION\_SECRET=a\_random\_value\_of\_at\_least\_32\_characters
```
...
* Channel analysis calculates every public upload locally, then supplies at most 12 Economy or 24 Heavy representative uploads to GPT.
...
* **Phase 1 — public evidence:** implemented for the Channel and video Dashboard, including complete channel-relative comparisons and percentiles.
